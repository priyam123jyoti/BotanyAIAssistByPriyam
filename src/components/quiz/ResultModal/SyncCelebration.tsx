...
export const SyncCelebration = ({ score, onComplete }: SyncCelebrationProps) => {
  ...
  // 🎵 UPDATED: Mapping the exact filenames you uploaded to the logic
  const getTierData = () => {
    if (score === 100) {
      return { 
        color: '#fbbf24', 
        audio: '/audio/hundred-percent.mp3', // Exact filename
        delay: 7000, 
    ...
    if (score >= 80) {
      return { 
        color: '#10b981', 
        audio: '/audio/eighty-nineghty-percent.mp3', // Exact filename
        delay: 5000, 
    ...
    if (score >= 40) {
      return { 
        color: '#3b82f6', 
        audio: '/audio/fourty-saventy-percenty.mp3', // Exact filename
        delay: 4500, 
        label: "UPLINK STABLE" 
      };
    }
    return { 
      color: '#ef4444', 
      audio: '/audio/below-thirty-percent.mp3', // Exact filename
      delay: 4000, 
  ...
  useEffect(() => {
    // Robust Audio Loading for Vercel
    ...
    const timeline = async () => {
      ...
      // Wait for the specific audio duration
